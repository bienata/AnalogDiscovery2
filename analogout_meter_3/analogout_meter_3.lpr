...
begin
  ...
  repeat
     ...
     Urms := 0.0;
     Udc := 0;
     //--- RMS ze składową stałą
(*
     for i := 0 to MAX_SAMPLES-1 do
     begin
         Urms := Urms + samplesBuff[ i ] * samplesBuff[ i ] ;
         Udc := Udc + samplesBuff[ i ];
     end;
     Urms := sqrt( Urms/MAX_SAMPLES );
     Udc := Udc/MAX_SAMPLES;
*)
     //--- RMS bez składowej stałej
     // samo DC
     ...
     writeln ( 'Urms=' + FormatFloat ('0.00', Urms) + ' Udc=' + FormatFloat ('0.00', Udc) ); // konsola
     Hdsp2111Text ( LeftStr('U:' + FormatFloat ('0.00', Urms)  + ' DC:' + FormatFloat ('0.00', Udc), 16) );
     // poslij na miernik
